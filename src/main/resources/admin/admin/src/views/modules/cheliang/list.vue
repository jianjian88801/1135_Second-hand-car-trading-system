        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('cheliang','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/ershouchejiaoyi/upload/cheliangMuBan.xls"
                        >批量导入汽车数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('cheliang','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="cheliangUploadSuccess"
                                :on-error="cheliangUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('cheliang','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "cheliang.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="cheliangNewMoney"
                                   header-align="center"
                                   label="价格">
                        <template slot-scope="scope">
                            {{scope.row.cheliangNewMoney}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="cheliangClicknum"
                                      header-align="center"
                                      label="点击次数">
                        <template slot-scope="scope">
                            {{scope.row.cheliangClicknum}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="zanNumber"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('cheliang','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('cheliang','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>

                            <el-button v-if="isAuth('cheliang','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <el-button v-if="isAuth('cheliang','修改')" type="primary" icon="el-icon-tickets" size="mini" @click="shangxia(scope.row.id,scope.row.shangxiaTypes)">{{scope.row.shangxiaTypes == 1?'下架':'上架'}}</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                ...
                cheliangPhoto : null,
                cheliangTypes : null,
                cheliangNewMoney : null,
                cheliangClicknum : null,
                zanNumber : null,
                caiNumber : null,
                cheliangContent : null,
                shangxiaTypes : null,
                cheliangDelete : null,
                createTime : null,
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'cheliang',//当前表表名,
                        sumColum : 'cheliang_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'cheliang_name',//分组字符串字段
                        // types : 'cheliang_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['cheliangDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            cheliangUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "cheliang/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
